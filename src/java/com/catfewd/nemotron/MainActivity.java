...
public class MainActivity extends Activity implements View.OnClickListener, android.widget.CompoundButton.OnCheckedChangeListener {
    ...
    private void showAdvancedSettings() {
        final String[] options = {
            getString(R.string.chunk_80ms),
            getString(R.string.chunk_160ms),
            getString(R.string.chunk_560ms)
        };
        final int[] values = {8, 16, 56};
        
        android.content.SharedPreferences prefs = getSharedPreferences("settings", MODE_PRIVATE);
        int currentVal = prefs.getInt("chunk_size", 56);
        int initialSelection = 2; // Default 560ms
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this, 
            prefs.getBoolean("dark_mode", false) ? android.R.style.Theme_DeviceDefault_Dialog_Alert : android.R.style.Theme_DeviceDefault_Light_Dialog_Alert);
        
        builder.setTitle(R.string.advanced_title);
        builder.setSingleChoiceItems(options, initialSelection, (dialog, which) -> {
            getSharedPreferences("settings", MODE_PRIVATE).edit().putInt("chunk_size", values[which]).apply();
        });
        
        builder.setPositiveButton(R.string.btn_ok, (dialog, which) -> {
            Toast.makeText(this, "Setting saved. Restart the keyboard to apply.", Toast.LENGTH_LONG).show();
        });

...
if [ "$GITHUB_ACTIONS" = "true" ]; then
    # GitHub Actions environment - Use Shared Linking to avoid 'Invalid record' errors in libdl.a
    ...
    export RUSTFLAGS="-L $NDK_CPP_DIR -L $GITHUB_WORKSPACE/jniLibs/arm64-v8a -C link-arg=-lc++_shared -C link-arg=-lonnxruntime -C link-arg=-Wl,-z,max-page-size=16384"
    ...
    # Local Termux environment - Use Monolithic Static Linking for maximum symbol stability
    AAPT2=$(which aapt2)
    D8=$(which d8)
...
if [ "$GITHUB_ACTIONS" = "true" ]; then
    # CI needs shared lib packaged
    CPPLIB=$(find $ANDROID_NDK_HOME -name "libc++_shared.so" | grep "aarch64" | head -n 1)
    cp "$CPPLIB" build_manual/lib/arm64-v8a/
else
    # Termux version is static, doesn't strictly need it, but we'll include it for other possible deps

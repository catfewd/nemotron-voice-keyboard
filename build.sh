...
if [ "$GITHUB_ACTIONS" = "true" ]; then
    ...
    # Path to official NDK libs in CI (populated by workflow)
    export RUSTFLAGS="-L $(pwd)/fake_libs -L $GITHUB_WORKSPACE/jniLibs/arm64-v8a -C link-arg=-lc++_static -C link-arg=-lc++abi -C link-arg=-lunwind -C link-arg=-lonnxruntime -C link-arg=-Wl,-z,max-page-size=16384"
    ...
    export RUSTFLAGS="-L $NDK_CPP_DIR -L $NDK_CLANG_DIR -L $(pwd)/fake_libs -L /data/data/com.termux/files/usr/lib -C link-arg=-lc++_static -C link-arg=-lc++abi -C link-arg=-lunwind -C link-arg=-lonnxruntime -C link-arg=-Wl,-z,max-page-size=16384"
...
# Note: We do NOT package libc++_shared.so because it's now monolithic/static!
